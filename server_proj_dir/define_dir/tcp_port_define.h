/*
  Copyrights reserved
  Written by Paul Hwang
  File name: tcp_port_define.h
*/

#pragma once

class TcpPortDefine {
public:
    unsigned short const static FABRIC_IPHONE_PORT_NUMER  = 8006;
    unsigned short const static FABRIC_ANDROID_PORT_NUMER = 8006;
    unsigned short const static FABRIC_TEST_PORT_NUMER    = 8006;
    unsigned short const static FABRIC_NODEJS_PORT_NUMER  = 8006;
    unsigned short const static FABRIC_THEME_PORT_NUMER   = 8007;
    unsigned short const static THEME_ENGINE_PORT_NUMER   = 8008;
};
