...
void *PortClass::receiveThreadFunction2 (void)
{
    while (1) {
        ...
        if (raw_data) {
            ...
            if (raw_data[0] == '{') {
                length = raw_length - (1 + TCP_IP_DEF::MAX_TCP_DATA_LEN_SIZE + 1);
                length1 = phwangDecodeNumber(&raw_data[1], TCP_IP_DEF::MAX_TCP_DATA_LEN_SIZE);
                if (length != length1) {
                ...
                memcpy(data, &raw_data[1 + TCP_IP_DEF::MAX_TCP_DATA_LEN_SIZE], length);
                data[length] = 0;
             }
            else {
